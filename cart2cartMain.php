 ...
 */
/*
Plugin Name: Cart2Cart: 3DCart to WooCommerce Migration Module
    ...
    case 'checkApi':
      if (!extension_loaded('soap')) {
        echo json_encode(array(
          'messages' => 'Cannot check without soap extension',
          'messageType' => 'success'
        ));
        exit();
      }
      $params = array(
        'storeUrl' => str_replace(array('http://', 'https://'), '', $_REQUEST['host']),
        'userKey' => $_REQUEST['pass']
      );
      ...
      $result = $db->runQuery($params + array(
          'sqlStatement' => "SELECT count(*) FROM category"
        ));

      $response = @simplexml_load_string($result->runQueryResult->any);
      ...
      switch ($response->getName()) {
        ...
        case  'runQueryResponse' :
          if (isset($response->Error)) {
            echo json_encode(array(
              'messages' => $response->Error->Description,
              'messageType' => 'error'
            ));
            exit();
          }
          if (!isset($response->runQueryRecord->Expr1000)) {
            echo json_encode(array(
              'messages' => 'Wrong responce from 3DCart server',
              'messageType' => 'success'
            ));
            exit();
          }
          echo json_encode(array(
            'messages' => 'Credentials are valid. You can proceed to the next step.',
            'messageType' => 'success'
          ));
          break;
        default :
      ...
      update_option('Cart2cartRemoteHost',$_REQUEST['Cart2cartRemoteHost']);
      update_option('Cart2cartRemotePassword',$_REQUEST['Cart2cartRemotePassword']);
      break;
    case 'installBridge':
...
function cart2cart_config(){
  ...
  $Cart2cartRemotePassword = get_option('Cart2cartRemotePassword');
