jQuery(document).ready(function($) {
  ...
  function checkApi(host,pass){
    console.log('try to check api');
    $.ajax({
      cache: false,
      url: SELF_PATH + "checkApi&host="+host+"&pass="+encodeURIComponent(pass),
  ...
  submitCart2cartRemoteForm.on("click",function(){
    ...
    Cart2cartRemotePassword.removeClass('c2cReqiredField');
    ...
    if (Cart2cartRemotePassword.val() == ''){
      Cart2cartRemotePassword.addClass('c2cReqiredField');
      $("#storeKey").html('Store key is required');
      $("#storeKey").animate({
        'opacity': 'show'
      });
      hasError = true;
  ...
  function doAction(action){
    switch (action){
      case 'installLocal':
        ...
        break
      case 'installRemote':
        checkApi(Cart2cartRemoteHost.val(),Cart2cartRemotePassword.val());
        break
    }
  }
